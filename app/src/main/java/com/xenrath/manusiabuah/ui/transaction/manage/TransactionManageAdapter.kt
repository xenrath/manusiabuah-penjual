...
import com.xenrath.manusiabuah.ui.transaction.manage.detail.TransactionDetailManageActivity

class TransactionManageAdapter(
    var context: Context,
    var transaction: ArrayList<DataTransaction>
) : RecyclerView.Adapter<TransactionManageAdapter.Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        holder.layoutInvoice.setOnClickListener {
            if (transaction.status == "Belum dibayar") {
                ...
                Constant.TRANSACTION_ID = transaction.id!!
                context.startActivity(Intent(context, TransactionDetailManageActivity::class.java))
            }
        }
    }
